...
logger = get_logger("speech_cloud")
...
class SpeechCloudWS(websocket.WebSocketHandler, EventEmitter):
    ...
    def open(self):
        logger.info("Creating dialog manager as instance of %s", self.dialog_class)
        self._task = None
        self.dm = self.dialog_class(self)

    def on_close(self):
        logger.info("Canceling dialog manager task (WebSocket closed)")
    ...
    def _check_task_result(self, task):
        try:
            ...
            logger.info("Dialog manager was canceled")
        except:
